...
export const createDateString = (dateStr: string) => {
  const travelDate = dayjs(dateStr).format(INPUT_DATE_FORMAT);
  return travelDate;
};

export const createDateStringTrip = (dateStr: string) => {
  if (!dateStr) {
    return;
  }
...
export const createDateFromTimeStamp = (timeStamp: any) => {
  if (!timeStamp) {
    return;
  }
...
export const getTimeInMins = (duration: number) => {
